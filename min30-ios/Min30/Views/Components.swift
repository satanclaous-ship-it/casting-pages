            ...
            ForEach(Array(labels.enumerated()), id: \.offset) { i, label in
                Button {
                    ...
                    Text(label)
                        ...
                        .frame(maxWidth: .infinity)
                        .padding(.vertical, 10)
                        .background(value == i ? Color.accentColor.opacity(0.16) : Color(.secondarySystemBackground),
                                    in: RoundedRectangle(cornerRadius: 12))
                        .overlay(
                            RoundedRectangle(cornerRadius: 12)
                                .strokeBorder(value == i ? Color.accentColor : .clear, lineWidth: 1)
                        )
                        .foregroundStyle(value == i ? Color.primary : Color.secondary)
                }
                .buttonStyle(.plain)
            }
        }
...
struct TagButton: View {
    ...
    var body: some View {
        Button(action: action) {
            HStack(spacing: 7) {
                Circle()
                    .fill(category?.color ?? Color.secondary)
